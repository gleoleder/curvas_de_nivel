...
function initCanvas2D() {
    ...
    canvas.addEventListener('mousemove', updateProfileLine);
...
function startProfile(e) {
    ...
    profileLine.end = { ...profileLine.start };
}

function updateProfileLine(e) {
...
function draw2D() {
    ...
    // Draw elevation gradient background
    for (let y = 0; y < resolution; y++) {
        for (let x = 0; x < resolution; x++) {
            const elev = terrainData[y][x];
            const color = getElevationColor(elev / maxHeight);
            ctx.fillStyle = color;
            ctx.fillRect(
                x * width / resolution,
                y * height / resolution,
                Math.ceil(width / resolution) + 1,
                Math.ceil(height / resolution) + 1
            );
        }
    }
    ...
    // Draw profile line
    if (profileLine.start && profileLine.end) {
        // Glow effect
        ...
        // Draw endpoints with gradient
        const endpointGrad = ctx.createRadialGradient(0, 0, 0, 0, 0, 10);
        endpointGrad.addColorStop(0, '#ff6b6b');
        endpointGrad.addColorStop(1, '#ff4444');
...
function getMarchingSquaresPoints(config, v0, v1, v2, v3, level, px, py, w, h) {
    ...
    const getT = (va, vb) => Math.max(0, Math.min(1, (level - va) / (vb - va)));
...
function drawContourLabels(ctx, level, width, height) {
    ...
    for (let y = resolution / 4; y < resolution * 3/4; y += resolution / 6) {
        for (let x = resolution / 4; x < resolution * 3/4; x += resolution / 6) {
            const v = terrainData[Math.floor(y)][Math.floor(x)];
...
function getElevationColor(t) {
    const colors = [
        { r: 15, g: 45, b: 65 },    // Deep blue-green (low)
        { r: 20, g: 80, b: 70 },    // Teal
        { r: 45, g: 120, b: 80 },   // Green
        { r: 100, g: 160, b: 90 },  // Light green
        { r: 180, g: 170, b: 100 }, // Tan
        { r: 160, g: 120, b: 80 },  // Brown
        { r: 130, g: 100, b: 90 },  // Dark brown
        { r: 200, g: 200, b: 210 }, // Light gray
        { r: 250, g: 250, b: 255 }  // Snow white (high)
    ];
...
function updateLegend(levels) {
    ...
    const masterLevels = levels.filter(l => l % (contourInterval * 5) === 0);
    masterLevels.slice(0, 6).forEach(level => {
        const item = document.createElement('div');
        item.className = 'legend-item';
        item.innerHTML = `
            <div class="legend-color" style="background: ${getElevationColor(level / maxHeight)}"></div>
            <span>${level}m</span>
        `;
...
function drawProfile() {
    ...
    // Sample elevations along profile line with higher resolution
    const samples = 200;
    const profileData = [];
    let maxElev = 0, minElev = maxHeight;
    ...
    for (let i = 0; i <= samples; i++) {
        ...
        const v00 = terrainData[Math.max(0, Math.min(resolution - 1, y0))][Math.max(0, Math.min(resolution - 1, x0))];
        const v10 = terrainData[Math.max(0, Math.min(resolution - 1, y0))][Math.max(0, Math.min(resolution - 1, x1))];
        const v01 = terrainData[Math.max(0, Math.min(resolution - 1, y1))][Math.max(0, Math.min(resolution - 1, x0))];
        const v11 = terrainData[Math.max(0, Math.min(resolution - 1, y1))][Math.max(0, Math.min(resolution - 1, x1))];
        
        const elev = v00 * (1-fx) * (1-fy) + v10 * fx * (1-fy) + v01 * (1-fx) * fy + v11 * fx * fy;
        
        profileData.push({ t, elev });
    ...
    // Draw terrain fill with smooth gradient
    ctx.beginPath();
    ...
    for (let i = 1; i < profileData.length; i++) {
        ...
        slopes.push(Math.abs(dElev / dDist * 100));
    }
    ...
    document.getElementById('statSlope').textContent = Math.max(...slopes).toFixed(1) + '%';
}
...
function initCanvas3D() {
    ...
    // Mouse controls - CORRECTED DIRECTION
    ...
    renderer.domElement.addEventListener('mousemove', (e) => {
        ...
        // CORRECTED: Inverted the signs for natural movement
        targetRotationY -= deltaX * 0.01;  // Changed from += to -=
        targetRotationX -= deltaY * 0.01;  // Changed from += to -=
    ...
    // Touch controls - CORRECTED DIRECTION
    let touchStartX, touchStartY, lastTouchDistance;
...
function update3D() {
    ...
    const exag = parseFloat(document.getElementById('verticalExag').value);
    ...
    for (let i = 0; i < res; i++) {
        for (let j = 0; j < res; j++) {
            const idx = (i * res + j) * 3;
            const elev = terrainData[i][j] / maxHeight * 0.5 * exag;
            vertices[idx + 2] = elev;
            
            const color = getVertexColor(terrainData[i][j] / maxHeight, i, j);
    ...
    // Add contour lines for 3D contour mode
    if (style === 'contours3d') {
        addContourLines3D();
    }
}

function getVertexColor(t, i, j) {
    const style = document.getElementById('renderStyle').value;
    
    if (style === 'slope') {
        let slope = 0;
        if (i > 0 && i < resolution - 1 && j > 0 && j < resolution - 1) {
            const dzdx = (terrainData[i][j+1] - terrainData[i][j-1]) / 2;
            const dzdy = (terrainData[i+1][j] - terrainData[i-1][j]) / 2;
            slope = Math.sqrt(dzdx*dzdx + dzdy*dzdy) / maxHeight * 5;
        }
        
        // Modern gradient: cyan to magenta
        return {
        ...
        const elev = t * maxHeight;
        const band = Math.floor(elev / contourInterval);
        const bandT = band / (maxHeight / contourInterval);
        ...
        const idx = Math.min(colors.length - 1, Math.floor(bandT * colors.length));
        return colors[idx];
    } else {
        ...
        if (i1 >= colors.length - 1) return colors[colors.length - 1];
        
        const c1 = colors[i1];
        const c2 = colors[i1 + 1];
        ...
        for (let y = 0; y < resolution - 1; y++) {
            for (let x = 0; x < resolution - 1; x++) {
                ...
                if ((v0 <= level && v1 > level) || (v0 > level && v1 <= level)) {
                    ...
                    const px = (x + t) / resolution * 2 - 1;
                    const py = y / resolution * 2 - 1;
                    const pz = level / maxHeight * 0.5 * exag + 0.003;
